...
//   // ===========================
//   // ✅ SEARCH QUESTION (functional)
//   // - Navigate to Home with query param: /?q=term
//   // ===========================
...
//         <form className="qd-search" onSubmit={submitSearch}>
//           <input
...
//           />
//           <button className="qd-search-btn" type="submit">
//             Search
//           </button>
//         </form>
...
//   // ✅ Welcome user (AuthContext first, then localStorage fallback)
//   const user = useMemo(() => {
...
//   // ===========================
//   // ✅ SEARCH QUESTION (functional)
//   // - uses same query param as Home: /?q=term
//   // ===========================
...
//     return `${BASE_URL}${avatarUrl}`; // http://localhost:5500/uploads/avatars/...
//   };
...
//             {qError ? (
//               <p className="qd-error qd-error-inline">{qError}</p>
//             ) : null}
...
export default function QuestionDetails() {
  ...
  const toAvatarUrl = (avatarUrl) => {
    if (!avatarUrl) return "/default-avatar.png"; // from public/
    if (avatarUrl.startsWith("http")) return avatarUrl;
    return `${BASE_URL}${avatarUrl}`; // http://localhost:5500/uploads/avatars/xxx.png
  };
  ...
  const addComment = async (answerid) => {
    ...
    try {
      ...
      // reload counts and answer list
      await load();

      // fetch comments again so they show immediately
      const res = await api.get(`/answers/${answerid}/comments`);
      setAnswers((prev) =>
        prev.map((a) =>
          a.answerid === answerid ? { ...a, comments: res.data || [] } : a
              ...
              return (
                <div key={key} className="qd-answer-item">
                  ...
                  <div className="qd-avatar">
                    <img
                      className="qd-avatar-img"
                      src={toAvatarUrl(a.avatar_url)}
                      alt={`${a.username || "User"} avatar`}
                      loading="lazy"
                      onError={(e) => {
